  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (purpose === 'signup') {
        ...
        setTimeout(() => navigate('/frontend_ai/reset-password', { state: { user_id } }), 2000);
      }
    } catch (err) {
      setError(err.response?.data?.errors || 'Invalid or expired OTP. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg w-full max-w-md">
        <div className="flex justify-center mb-6">
          <img src="/src/assets/closetai-logo.jpg" alt="ClosetAI Logo" className="h-12 md:h-14 w-auto transition-opacity hover:opacity-90" />
        </div>
        <h2 className="text-2xl md:text-3xl font-bold text-gray-800 text-center mb-2">Verify OTP</h2>
